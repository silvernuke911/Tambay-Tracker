...
def main():
    prompt = entry.starting_menu(raw_data_file, score_file, date_file, valid_names) 
    if prompt in ['y','']:
        entry.get_entry_input(raw_data_file, score_file, date_file, valid_names)
    while True:
        option_choice = query.get_option_input(program_options, raw_data_file, score_file, date_file, valid_names)
        entry.handle_option_choice(option_choice, raw_data_file, score_file, date_file, valid_names, member_file)
